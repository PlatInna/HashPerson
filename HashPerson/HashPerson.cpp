// HashPerson.cpp : https://www.coursera.org/ C++ Development Fundamentals: Brown Belt, Week 1.
// task: Integrat structure Person into unordered_set
...
struct Address {
    ...
    bool operator==(const Address& other) const {
        return city == other.city 
           and street == other.street 
           and building == other.building;
    }
};
...
struct AddressHasher {
    size_t operator() (const Address& a) const {
        //Ax^2+Bx+C - hasher
        size_t x = 97;
        size_t A = str_hasher(a.city);
        size_t B = str_hasher(a.street);
        size_t C = int_hasher(a.building);
        return (A * x * x + B * x + C);
    }
...
struct PersonHasher {
    size_t operator() (const Person& p) const {
        //Ax^3+Bx^2+Cx+D - hasher
        size_t x = 97;
        size_t A = str_hash(p.name);
        size_t B = int_hash(p.height);
        size_t C = double_hash(p.weight);
        size_t D = address_hash(p.address);
...
void TestSmoke() {
    ...
    //vector<Person> points = {  <------------------------- VS cannot initialize a vector this way
